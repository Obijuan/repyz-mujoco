...
def main():
    ...
    parser.add_argument("--viewer", type=str, choices=["native", "viser"],
                        default="native",
                        help="Select the viewer (native or viser)")
    args = parser.parse_args()

    # Load the model
    model = mujoco.MjModel.from_xml_path(  # type: ignore
                "assets/minicube_1_repyz.xml")
    ...
    fig = mujoco.MjvFigure()  # type: ignore
    history_len = 500
    history = deque(maxlen=history_len)

    if args.viewer == "native":
        mujoco.mjv_defaultFigure(fig)  # type: ignore
        fig.flg_extend = 1
    ...
    if args.viewer == "native":
        print("Press the 'p' key in the simulation window to show/hide "
              "the graph.")
    else:
        print("Viser web viewer activated. Open your web browser at the "
              "indicated address.")
    ...
    def run_simulation_loop(viewer, get_show_plot):
        ...
        actuator_id = mujoco.mj_name2id(  # type: ignore
            model,
            mujoco.mjtObj.mjOBJ_ACTUATOR,  # type: ignore
            "servo_actuator")

        actuator_id2 = mujoco.mj_name2id(  # type: ignore
            model,
            mujoco.mjtObj.mjOBJ_ACTUATOR,  # type: ignore
            "servo_actuator2")
        ...
        while is_running():
            ...
            amplitude = 0.8
            frequency = 1.0
            phase = 2 * math.pi * frequency * data.time

            # -- Move the first module
            if actuator_id != -1:
                val = amplitude * math.sin(phase) + offset
            ...
            if actuator_id2 != -1:
                ...
                # -- Phase difference of 120 deg generates a good coordination
                phase_diff = -2 * math.pi/3

                val2 = amplitude * math.sin(phase + phase_diff) + offset
